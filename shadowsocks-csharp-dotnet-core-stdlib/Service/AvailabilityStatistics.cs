    ...
    public sealed class AvailabilityStatistics : IDisposable
    {
        ...
        private void UpdateSpeed()
        {
            foreach (var kv in _inOutBoundRecords)
            {
                ...
                _logger.Debug($"{id}: current/max inbound {inboundSpeed}/{inR.Max()} KiB/s, current/max outbound {outboundSpeed}/{outR.Max()} KiB/s");
            }
        }
        ...
        private void UpdateRecords()
        {
            ...
            for (int i = 0; i < serverCount; i++)
            {
                ...
                {
                    _logger.Debug("config changed asynchrously, just ignore this server");
                }
            }
        ...
        private void Ping_Completed(object sender, MyPing.CompletedEventArgs e)
        {
            ...
            _logger.Debug($"Ping {server.FriendlyName()} {e.RoundtripTime.Count} times, {(100 - record.PackageLoss * 100)}% packages loss, min {record.MinResponse} ms, max {record.MaxResponse} ms, avg {record.AverageResponse} ms");
        ...
        private void Save()
        {
            _logger.Debug($"save statistics to {AvailabilityStatisticsFile}");
        ...
        private void FilterRawStatistics()
        {
            try
            {
                _logger.Debug("filter raw statistics");
        ...
        private void LoadRawStatistics()
        {
            try
            {
                ...
                _logger.Debug($"loading statistics from {path}");
                if (!File.Exists(path))
                {
        ...
        private class UpdateRecordsState
        {
            public int counter;
        }
        ...
        private class MyPing
        {
            ...
            private readonly List<int?> RoundtripTime;
            ...
            private void ICMPTest(int delay, object userstate)
            {
                try
                {
                    _logger.Debug($"Ping {server.FriendlyName()}");
                    if (ip == null)
                    {
                ...
                {
                    _logger.Error($"An exception occured while eveluating {server.FriendlyName()}");
                    _logger.LogUsefulException(e);
            ...
            private void Ping_PingCompleted(object sender, PingCompletedEventArgs e)
            {
                try
                {
                    ...
                    {
                        _logger.Debug($"Ping {server.FriendlyName()} {e.Reply.RoundtripTime} ms");
                        RoundtripTime.Add((int?)e.Reply.RoundtripTime);
                    }
                    else
                    {
                        _logger.Debug($"Ping {server.FriendlyName()} timeout");
