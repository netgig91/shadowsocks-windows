...
namespace Shadowsocks.Std.Service
{
    public class UpdateChecker
    {
        ...
        public void CheckUpdate(Configuration config)
        {
            ...
            try
            {
                _logger.Debug("Checking updates...");
        ...
        private void Http_DownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            try
            {
                ...
                {
                    _logger.Debug("No update is available");
        ...
        private void Http_DownloadFileCompleted(object sender, System.ComponentModel.AsyncCompletedEventArgs e)
        {
            try
            {
                if (e.Error != null)
                {
                    _logger.LogUsefulException(e.Error);
                    return;
                }
                _logger.Debug($"New version {LatestVersionNumber}{LatestVersionSuffix} found: {LatestVersionLocalName}");
                CheckUpdateCompleted?.Invoke(this, new EventArgs());
            }
            catch (Exception ex)
            {
        ...
        public class Asset : IComparable<Asset>
        {
            ...
            public int CompareTo(Asset other) => CompareVersion(version, other.version);
        }
    }
}