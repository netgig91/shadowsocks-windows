    ...
    public class ShadowsocksController
    {
        ...
        public EndPoint GetPluginLocalEndPointIfConfigured(Server server)
        {
            ...
            try
            {
                ...
                {
                    _logger.Info($"Started SIP003 plugin for {server.Identifier()} on {plugin.LocalEndPoint} - PID: {plugin.ProcessId}");
                }
            }
            catch (Exception ex)
            {
                _logger.Error("Failed to start SIP003 plugin: " + ex.Message);
                throw;
            }
